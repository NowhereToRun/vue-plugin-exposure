...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["@mfelibs/vue-plugin-exposure"] = factory();
	else
		root["@mfelibs/vue-plugin-exposure"] = factory();
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__plugin__ = __webpack_require__(2);
/* harmony reexport (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return __WEBPACK_IMPORTED_MODULE_0__plugin__["a"]; });
...
var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function");}}var io=void 0;// 所有指令可以使用同一个io
var defaultIOConfig={threshold:[0],root:null,rootMargin:"0px 0px -50px 0px"};var ExpSet=function(){function ExpSet(){_classCallCheck(this,ExpSet);this.prefix=(+new Date()).toString(36);this.seed=0;this.set={};}_createClass(ExpSet,[{key:'uuid',value:function uuid(){return'exposureset_'+this.prefix+'_'+this.seed++;}},{key:'add',value:function add(cb){var uuid=this.uuid();this.set[uuid]=cb;return uuid;}},{key:'has',value:function has(uuid){return!!this.set[uuid];}},{key:'get',value:function get(uuid){return this.set[uuid];}},{key:'delete',value:function _delete(uuid){delete this.set[uuid];}}]);return ExpSet;}();var expSet=new ExpSet();/* harmony default export */ __webpack_exports__["a"] = ({install:function install(Vue){var defaultOptions=arguments.length>1&&arguments[1]!==undefined?arguments[1]:{};var _defaultOptions$confi=defaultOptions.config,config=_defaultOptions$confi===undefined?{}:_defaultOptions$confi,_defaultOptions$name=defaultOptions.name,pluginName=_defaultOptions$name===undefined?'exposure':_defaultOptions$name;var observerConfig=Object.assign({},defaultIOConfig,config);console.log('[exposure plugin] '+pluginName+' config',observerConfig);if(!io){io=new IntersectionObserver(function(entries){for(var i=0,len=entries.length;i<len;i++){var entry=entries[i];if(entry.isIntersecting){console.log(entry);var cbId=entry.target.dataset.cbid;if(expSet.has(cbId)){try{expSet.get(cbId)();}catch(e){console.log('执行cb出错',entry,e);}}expSet.delete(cbId);io.unobserve(entry.target);}}},observerConfig);}var directionName=pluginName.replace(/^v-/,'');Vue.directive(directionName,{bind:function bind(el,binding,vnode,oldVnode){if(typeof binding.value==='function'){var cbId=expSet.add(binding.value);el.dataset.cbid=cbId;}io.observe(el);},unbind:function unbind(el,binding,vnode,oldVnode){expSet.delete(el.dataset.cbid);io.unobserve(el);}});}});
